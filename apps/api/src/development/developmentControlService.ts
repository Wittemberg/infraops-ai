...
export class DevelopmentControlService {
  ...
  private loadJson<T>(filename: string): T {
    ...
    if (!existsSync(filePath)) {
      throw new Error(`[DevelopmentControlService] Canonical data file not found: ${filePath}`);
    }
    try {
      const raw = readFileSync(filePath, "utf-8");
      return JSON.parse(raw) as T;
    } catch (err: any) {
      throw new Error(`[DevelopmentControlService] Error parsing JSON file ${filename}: ${err?.message || err}`);
    }
  }
